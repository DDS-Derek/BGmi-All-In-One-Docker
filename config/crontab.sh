...
if [ $? -eq 0 ]; then
    ...
    (crontab -l ; echo "0 */2 * * * su bgmi -c 'LC_ALL=en_US.UTF-8 $BGMI_PATH update $DOWNLOAD'") | crontab -
...
if [ $? -eq 0 ]; then
    ...
    (crontab -l ; echo "0 */10 * * * su bgmi -c 'LC_ALL=en_US.UTF-8 TRAVIS_CI=1 $BGMI_PATH cal --force-update --download-cover'") | crontab -
